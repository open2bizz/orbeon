...
class OrbeonRequestHandler(object):
    ...
    def __init__(self, request, configfile_path=None, wsgi_input=None):
        _logger.error("OrbeonRequestHandler.__init__: ENTER request=%r path=%s",
        ...
        _logger.error("OrbeonRequestHandler.__init__: using configfile_path=%s", configfile_path)
        ...
        _logger.error("OrbeonRequestHandler.__init__: raw path=%s normalized=%s split=%s args=%s persistence=%s",
        ...
        try:
            if request.headers.get('Transfer-Encoding', '') == 'chunked' and request.headers.get('Content-Length', ' ') != ' ':
                _logger.error("OrbeonRequestHandler.__init__: handling chunked request body")
                body = b''
                if wsgi_input is not None:
                    ...
                    try:
                        ...
                        _logger.error("OrbeonRequestHandler.__init__: chunked size parse error=%s line=%r", e, size_line)
                        size = 0
                    while size > 0:
                        ...
                        try:
                            ...
                            _logger.error("OrbeonRequestHandler.__init__: chunked size loop parse error=%s", e)
                            size = 0
                self.data = body
                _logger.error("OrbeonRequestHandler.__init__: chunked body length=%d", len(self.data or b""))
            else:
                self.data = request.data
                _logger.error("OrbeonRequestHandler.__init__: non-chunked body length=%d", len(self.data or b""))
        except Exception as e:
            _logger.error("OrbeonRequestHandler.__init__: ERROR reading body: %s", e, exc_info=True)
            self.data = b""
        ...
        try:
            if self.handler is None:
                _logger.error("OrbeonRequestHandler.__init__: no handler for namespace=%s; skip backend config", self.namespace)
            elif self.handler_type in (BUILDER_HANDLER, RUNNER_HANDLER, ODOO_SERVICE_HANDLER):
                if configfile_path is not None:
                    _logger.error("OrbeonRequestHandler.__init__: set_config_by_file_path(%s)", configfile_path)
                ...
                if getattr(self.handler, "config", None) is not None and len(self.handler.config.sections()) > 0:
                    _logger.error("OrbeonRequestHandler.__init__: using XML-RPC from config (sections=%d)", len(self.handler.config.sections()))
                    self.handler.set_xmlrpc_by_config(request)
                else:
                    _logger.error("OrbeonRequestHandler.__init__: building XML-RPC from headers")
                    url = "http://%s:%s" % (
                        request.headers.get("Openerp-Server"),
                        request.headers.get("Openerp-Port"),
                    )
                    db = request.headers.get("Openerp-Database")
                    _logger.error("OrbeonRequestHandler.__init__: xmlrpc url=%s db=%s", url, db)

                    # Preferred: credentials from Odoo config
                    usr = str(config.get("orbeon_user"))
                    passwd = str(config.get("orbeon_password"))
                    _logger.error("OrbeonRequestHandler.__init__: credentials source=odoo.config has_user=%s has_password=%s",
                    ...
                    if auth_hdr and auth_hdr.startswith("Basic "):
                        _logger.error("OrbeonRequestHandler.__init__: Authorization header present; attempting decode of username only")
                        try:
                            ...
                            if p:
                                passwd = p
                            _logger.error("OrbeonRequestHandler.__init__: Authorization username resolved user=%s (password masked)", usr)
                        except Exception as e:
                            _logger.error("OrbeonRequestHandler.__init__: Authorization decode error: %s", e, exc_info=True)

                    # NEVER log credentials content
                    self.handler.set_xmlrpc(db, usr, passwd, url)
                    _logger.error("OrbeonRequestHandler.__init__: set_xmlrpc completed")
            else:
                _logger.error("OrbeonRequestHandler.__init__: handler doesn't require XML-RPC (type=%s)", self.handler_type)
        except Exception as e:
            _logger.error("OrbeonRequestHandler.__init__: ERROR configuring XML-RPC: %s", e, exc_info=True)
        _logger.error("OrbeonRequestHandler.__init__: EXIT")
    ...
    def set_path_attrs(self):
        ...
        """
        _logger.error("set_path_attrs: ENTER with path=%s", self.path)
        try:
            ...
            _logger.error("set_path_attrs: namespace=%s app=%s form=%s data_type=%s",
                          self.namespace, self.app, self.form, self.data_type)
        except Exception as e:
            _logger.error("set_path_attrs: ERROR parsing path components: %s", e, exc_info=True)
        _logger.error("set_path_attrs: EXIT")

    def set_handler_type(self):
        """Get Orbeon handler type (class)"""
        _logger.error("set_handler_type: ENTER namespace=%s app=%s form=%s persistence_pref=%s",
        ...
        try:
            ...
            # /form/... ONLY when coming via persistence prefix -> Forms Metadata API (v1)
                ...
                self.handler_type = None
            _logger.error("set_handler_type: resolved handler_type=%s", self.handler_type)
        except Exception as e:
            _logger.error("set_handler_type: ERROR resolving handler type: %s", e, exc_info=True)
        _logger.error("set_handler_type: EXIT")

    def set_handler(self):
        """Instantiate the proper handler based on handler_type"""
        _logger.error("set_handler: ENTER handler_type=%s", self.handler_type)
        try:
            if self.handler_type == BUILDER_HANDLER:
                ...
                _logger.error("set_handler: No handler for namespace=%s app=%s form=%s data_type=%s -> None",
                              self.namespace, self.app, self.form, self.data_type)
                self.handler = None
            _logger.error("set_handler: instantiated handler=%r",
                          (type(self.handler).__name__ if self.handler else None))
        except Exception as e:
            _logger.error("set_handler: ERROR instantiating handler: %s", e, exc_info=True)
            raise
        _logger.error("set_handler: EXIT")

    def _wrap_response(self, payload, default_mimetype="text/plain", status=200):
        """Normalize handler outputs into a Werkzeug Response."""
        _logger.error("_wrap_response: ENTER status=%s default_mimetype=%s payload_type=%s",
                      status, default_mimetype, type(payload).__name__)
        try:
            if isinstance(payload, Response):
                _logger.error("_wrap_response: payload already Response; EXIT")
                return payload
            if payload is None:
                payload = ""  # Avoid NoneType downstream
            resp = Response(payload, status=status, mimetype=default_mimetype)
            _logger.error("_wrap_response: created Response len=%s", len(payload) if hasattr(payload, "__len__") else "n/a")
            return resp
        except Exception as e:
            _logger.error("_wrap_response: ERROR creating response: %s", e, exc_info=True)
            return Response("Internal Server Error", status=500, mimetype="text/plain")

    def process(self):
        """Call the handler's HTTP-method equivalent function and ALWAYS return Response"""
        _logger.error("process: ENTER method=%s path=%s args=%s",
        ...
        if getattr(self, "handler", None) is None:
            _logger.error("process: No handler mapped for this namespace; returning 404")
        ...
        is_head = False
        if method == "HEAD":
            _logger.error("process: HEAD detected; routing as GET and stripping body later")
            is_head = True
            method = "GET"

        try:
            if method == "PUT":
                _logger.error("process: PUT -> handler.save()")
                result = self.handler.save()
                _logger.error("process: PUT save() result type=%s len=%s", type(result).__name__, (len(result) if result else 0))
                if result in (None, b"", ""):
                    _logger.error("process: PUT returning 204 No Content")
                    return Response("", status=204)
                mimetype = "application/xml" if (self.data_type == "data") else "text/plain"
                _logger.error("process: PUT returning 200 with mimetype=%s", mimetype)
                return self._wrap_response(result, default_mimetype=mimetype, status=200)

            if method == "GET":
                _logger.error("process: GET -> handler.read()")
                res = self.handler.read()

                # If handler returned a Response, just pass it through (don't call len())
                if isinstance(res, Response):
                    _logger.error("process: GET handler returned Response; passthrough status=%s mimetype=%s",
                ...
                try:
                    ...
                    res_len = 0
                _logger.error("process: GET read() result type=%s len=%s", type(res).__name__, res_len)

                # Decide content type
                if self.handler_type == ODOO_SERVICE_HANDLER:
                    _logger.error("process: GET returning application/xml for ODOO_SERVICE_HANDLER")
                    return self._wrap_response(res, default_mimetype="application/xml")
                if self.data_type == 'data':
                    _logger.error("process: GET returning application/xml for data_type=data")
                    return self._wrap_response(res, default_mimetype="application/xml")
                if self.data_type == 'form':
                    _logger.error("process: GET returning application/xhtml+xml for data_type=form")
                    return self._wrap_response(res, default_mimetype="application/xhtml+xml")
                # Fallback
                _logger.error("process: GET returning text/plain fallback")
                return self._wrap_response(res, default_mimetype="text/plain")

            if method == "POST":
                _logger.error("process: POST -> handler.search() with SEARCH_MIMETYPE=%s", SEARCH_MIMETYPE)
                criteria_bytes = self.data or b""
                _logger.error("process: POST payload length=%d", len(criteria_bytes))
                if SEARCH_MIMETYPE == "application/xml":
                    try:
                        ET.fromstring(criteria_bytes or b"<empty/>")
                        _logger.error("process: POST XML payload well-formed")
                    except ET.ParseError as e:
                        _logger.error("process: POST Invalid XML: %s", e, exc_info=True)
                        return Response(f"Invalid XML: {e}", status=400, mimetype="text/plain")
                res = self.handler.search()
                _logger.error("process: POST search() result type=%s len=%s", type(res).__name__, (len(res) if res else 0))
                return self._wrap_response(res, default_mimetype=SEARCH_MIMETYPE)

            if method == "DELETE":
                _logger.error("process: DELETE")
                if hasattr(self.handler, "delete"):
                    _logger.error("process: DELETE -> handler.delete()")
                    res = self.handler.delete()
                    _logger.error("process: DELETE result type=%s len=%s", type(res).__name__, (len(res) if res else 0))
                    if res in (None, b"", ""):
                        _logger.error("process: DELETE returning 204 No Content")
                        return Response("", status=204)
                    _logger.error("process: DELETE returning 200 text/plain")
                    return self._wrap_response(res, default_mimetype="text/plain")
                _logger.error("process: DELETE not supported -> 405")
                return Response("Method Not Allowed", status=405, mimetype="text/plain")

            _logger.error("process: unsupported method=%s -> 405", method)
            return Response("Method Not Allowed", status=405, mimetype="text/plain")
        except Exception as e:
            _logger.error("process: ERROR handling method=%s: %s", method, e, exc_info=True)
            return Response("Internal Server Error", status=500, mimetype="text/plain")
        finally:
...
class OrbeonPersistenceApp(object):
    def __init__(self, configfile_path=None):
        _logger.error("OrbeonPersistenceApp.__init__: ENTER configfile_path=%s", configfile_path)
        self.configfile_path = configfile_path
        _logger.error("OrbeonPersistenceApp.__init__: EXIT")

    def dispatch_request(self, request, wsgi_input=None):
        _logger.error("dispatch_request: ENTER method=%s path=%s", request.method, request.path)
        try:
            orbeon_request = OrbeonRequestHandler(request, self.configfile_path, wsgi_input)
            resp = orbeon_request.process()
            _logger.error("dispatch_request: got Response status=%s", getattr(resp, "status", None))
            return resp
        except Exception as e:
            _logger.error("dispatch_request: ERROR %s", e, exc_info=True)
            return Response("Internal Server Error", status=500, mimetype="text/plain")
        finally:
            _logger.error("dispatch_request: EXIT")

    def wsgi_app(self, environ, start_response):
        _logger.error("wsgi_app: ENTER environ PATH_INFO=%s REQUEST_METHOD=%s", environ.get("PATH_INFO"), environ.get("REQUEST_METHOD"))
        try:
            request = Request(environ)
            _logger.debug("request: %s %s", request.method, request.path)
            _logger.error("wsgi_app: constructed Request method=%s path=%s headers_keys=%s",
            ...
            if not isinstance(response, Response):
                _logger.error("wsgi_app: Invalid response type from process(): %r; coercing to 500", type(response))
            ...
            if request.method.upper() == "HEAD":
                _logger.error("wsgi_app: HEAD request -> strip body & set Content-Length: 0")
                response.headers.pop("Content-Length", None)
                response.headers["Content-Length"] = "0"
                response.set_data(b"")

            _logger.debug("response: %s", response.status)
            _logger.error("wsgi_app: EXIT status=%s", response.status)
            return response(environ, start_response)
        except Exception as e:
            _logger.error("wsgi_app: ERROR %s", e, exc_info=True)
            resp = Response("Internal Server Error", status=500, mimetype="text/plain")
            return resp(environ, start_response)

    def __call__(self, environ, start_response):
        _logger.error("__call__: delegating to wsgi_app")
        return self.wsgi_app(environ, start_response)


def create_app(configfile_path=None):
    _logger.error("create_app: ENTER configfile_path=%s", configfile_path)
    app = OrbeonPersistenceApp(configfile_path)
    _logger.error("create_app: EXIT returning OrbeonPersistenceApp")
    return app
